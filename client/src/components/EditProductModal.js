...
class EditProductModal extends Component {
  ...
  handleEditProduct = e => {
    ...
    const { id } = this.props;
    this.props.dispatch(editProduct({id, productName, description, quantity}));
    this.toggle();
  }
  ...
  render() {
    ...
    return (
      <div>
        <Button variant="contained" color="primary" className={classes.button} onClick={this.toggle}>Edit Product</Button>
        <Modal isOpen={modalOpen} toggle={this.toggle}>
          <ModalHeader toggle={this.toggle}>
            <div className="container text-center">
               <div style={{marginTop: '1 rem'}}>Edit Product</div>
            </div>
          </ModalHeader>
          <ModalBody>
            <ValidatorForm ref="form" className={classes.form} onSubmit={this.handleEditProduct} onError={errors => console.log(errors)}>
              <TextValidator
                label="Product Name"
...
export default connect(mapStateToProps, null)(withStyles(styles)(EditProductModal));
